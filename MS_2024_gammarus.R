...
#setwd('C:/Users/kaalt/Desktop/Lobster Manuscript/MS_Lobster_Plots')
...
#TL to CL conversion coefficients (Pavicic et al 2021)
  #female: TL = 32.893+2.617 × CL  -->  CL = 0.382 x TL - 12.569
  #male: TL = 42.351+2.467 × CL    -->  CL = 0.405 x TL - 17.167
inter <- c(-12.569,-17.167)  #c(-6.84,-10.91)
slop <- c(0.382,0.405)  #c(0.38,0.40)
...
for(g in 1:2) { 
  ...
  for (a in 1:(Amax[1]-1)) {   #for each cohort
    if (a <= Amax[1]) {          
      ...
      if(g==1){
        if (L[a, g] > 86.9 & L[a, g] < 114.76) {        #Females  
          select[a,g] = 1
        } else select[a,g] = 0
      }
      if(g==2){
        if (L[a, g] > 88.6 & L[a, g] < 117.09) {         #Males 
          select[a,g] = 1   
        }else select[a,g] = 0
      }
...
dev.off()
ratiofig(value=Sexratio, ylbl="Proportion Female",name="gam_SexRatio.pdf",leg=0.2)
dev.off()
...
f3and9 <- ggarrange(s03+theme(axis.text.x = element_blank(), axis.title.x = element_blank(), axis.title.y = element_blank()),
                    ...
                    ncol=2, nrow=3, hjust=-1.1, align = c("v"),
...
pdf("gam_size_nofish_im_mat.pdf",  width = 14, height = 15,  )
nofish_im_mat <- ggarrange(sim00,s00,
                           ncol=1, nrow=2, common.legend = TRUE, legend = "bottom",
                           labels=c("C","D"), hjust=-7, font.label = list(size=20))
nofish_im_mat
...
sizefig3(v=1:20,name = "gam_size_0avg_dimorphism.pdf") #no female protections
dev.off()
sizefig3(v=21:40,name = "gam_size_1avg_dimorphism.pdf") #vmax=1
dev.off()
sizefig3(v=41:60,name = "gam_size_4avg_dimorphism.pdf") #vmax=4
dev.off()
...
ggplot(datdif,aes(x=Fishing,y=value,color=variable,group=variable))+
  ...
  ggtitle("H. gammarus")+
  theme(plot.title = element_text(color="black", size=14, face="italic"))+
  theme(legend.position = c(0.17, 0.25),legend.background = element_rect(fill = "white"))
dev.off()
